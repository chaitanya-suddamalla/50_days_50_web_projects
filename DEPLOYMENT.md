...
└── public/                 # All included projects (50)
    ├── Login.html
    ├── signup.html
    └── [project folders]/
```
...
```bash
# Build and run
docker build -t 50days-web .
docker run -p 8080:80 50days-web
...
```bash
docker pull ghcr.io/chaitanya-suddamalla/50_days_50_web_projects:latest
docker run -p 8080:80 ghcr.io/chaitanya-suddamalla/50_days_50_web_projects:latest
```
